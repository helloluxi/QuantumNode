...
class GroupInputNode(Node, LinearQuantumCircuitTreeNode):
    # Optional identifier string. If not explicitly defined, the python class name is used.
    bl_idname = 'qnn_GroupInput'
    # Label for nice name display
    bl_label = "Group Input"
    # Icon identifier
    bl_icon = 'NEXT_KEYFRAME'
    ...
    def init(self, context):
        for i in range(self.outputSize):
            self.outputs.new('qns_SingleQubit' if i < self.boolOutputSize else 'qns_MultiQubits', getattr(self, f'output{i+1}'), identifier=f'o{i}')
