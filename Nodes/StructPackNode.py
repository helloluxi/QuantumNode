...
class StructPackNode(Node, LinearQuantumCircuitTreeNode):
    # Optional identifier string. If not explicitly defined, the python class name is used.
    bl_idname = 'qnn_StructPack'
    # Label for nice name display
    bl_label = "Struct Pack"
    # Icon identifier
    ...
    def refresh(self, context):
        if len(self.inputs) > self.inputSize:
            ...
            if self.outputs[0].name != self.structName:
                to_sockets = [link.to_socket for link in self.outputs[0].links]
                self.outputs.clear()
                self.outputs.new(socket.bl_idname, self.structName)
                for to_socket in to_sockets:
                    self.id_data.links.new(self.outputs[0], to_socket)
    ...
    def init(self, context):
        for i in range(self.inputSize):
            self.inputs.new('qns_SingleQubit' if i < self.boolInputSize else 'qns_MultiQubits', getattr(self, f'input{i+1}'))
        self.outputs.new('qns_MultiQubits', self.structName)
